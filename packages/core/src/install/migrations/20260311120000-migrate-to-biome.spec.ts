...
function normalizeBiomeSchemaVersion(content: string): string {
  return content.replace(
    BIOME_SCHEMA_URL_PATTERN,
    "https://biomejs.dev/schemas/__VERSION__/schema.json",
  );
}
...
describe("Migration 20260311120000-migrate-to-biome", () => {
  ...
  describe("Up", () => {
    ...
    it("should keep invalid existing biome config files unchanged", async () => {
      const biomeConfigFilePath = join(testProjectDir, "biome.json");
      ...
      expect(FileService.getFileContent(biomeConfigFilePath)).toBe(
        invalidBiomeConfig,
      );
    });
    ...
    it("should continue when biome init does not create a config file", async () => {
      ...
      expect(FileService.fileExists(join(testProjectDir, "biome.json"))).toBe(
        false,
      );
      expect(
    ...
    it("should not set vcs.root when no enclosing git directory exists", async () => {
      const biomeConfigFilePath = join(testProjectDir, "biome.json");
      FileService.putFileContent(
        biomeConfigFilePath,
        '{\n  "formatter": {\n    "enabled": true\n  }\n}\n',
      );
      rmSync(join(testProjectDir, ".git"), { force: true, recursive: true });

      await up(testProjectDir);

      expect(
        JSON.parse(FileService.getFileContent(biomeConfigFilePath)),
      ).toMatchObject({
        vcs: {
          clientKind: "git",
          enabled: true,
      ...
      expect(
        JSON.parse(FileService.getFileContent(biomeConfigFilePath)).vcs.root,
      ).toBeUndefined();
    });

    it("should set vcs.root when migrating a subpackage", async () => {
      const packageDir = join(testProjectDir, "packages", "test-package");
      ...
      expect(
        JSON.parse(FileService.getFileContent(join(packageDir, "biome.json"))),
      ).toMatchObject({
        vcs: {
          root: "../..",
        },
    ...
    it("should keep unrelated source files unchanged", async () => {
      ...
      FileService.putFileContent(appFilePath, appFileContent);

      await up(testProjectDir);

      expect(FileService.getFileContent(appFilePath)).toBe(appFileContent);
    });

    it("should tolerate managed eslint config removal during migration", async () => {
      const eslintConfigFilePath = join(testProjectDir, "eslint.config.mjs");
      FileService.putFileContent(
        eslintConfigFilePath,
        `const tsDevToolsCore = [];
    ...
    it("should migrate the common vite react-ts starter to biome-compatible code", async () => {
      ...
      FileService.putFileContent(
        ...
        [
          ...
          "        Vite",
          "      </a>",
          '      <a href="https://react.dev" target="_blank" className="logo" rel="noreferrer">',
          "        React",
          "      </a>",
