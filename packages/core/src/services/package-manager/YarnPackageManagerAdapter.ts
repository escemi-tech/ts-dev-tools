...
export class YarnPackageManagerAdapter extends AbstractPackageManagerAdapter {
    ...
    try {
      ...
      if (
        ...
        const retryArgs = args
          .filter(
            (arg) => arg !== "--ignore-workspace-root-check" && arg !== "-W",
          )
  ...
  private analyzeYarnWorkspacesOutput(output: string): {
    hasEntries: boolean;
    hasRecognizedData: boolean;
  } {
  ...
  private hasWorkspaceMap(value: unknown): boolean {
    ...
    const workspaces = (value as { workspaces?: Record<string, unknown> })
      .workspaces;
  ...
  private yarnListOutputHasPackage(
    ...
    for (const entry of entries) {
      ...
      const data = (entry as { data?: { trees?: Array<{ name: string }> } })
        .data;
      const trees =
        data?.trees ?? (entry as { trees?: Array<{ name: string }> }).trees;

      if (!trees) {
        const children = (entry as { children?: Record<string, unknown> })
          .children;
        if (children) {
