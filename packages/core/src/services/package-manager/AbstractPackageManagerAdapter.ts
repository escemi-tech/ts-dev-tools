...
export abstract class AbstractPackageManagerAdapter
  implements PackageManagerAdapter
{
  ...
  protected async getVersion(
    ...
    const raw = (
      await this.execCommand([packageManager, "--version"], dirPath, true)
    ).trim();
  ...
  protected async execCommand(
    ...
    return new Promise((resolve, reject) => {
      ...
      const rejectOnce = (reason: Error | string) => {
        if (isSettled) {
          return;
        }
