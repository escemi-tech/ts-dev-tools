...
import { up as coreInit } from "@ts-dev-tools/core/dist/install/migrations/20201024173398-init";
import { up as coreMigrateToBiome } from "@ts-dev-tools/core/dist/install/migrations/20260311120000-migrate-to-biome";
import { up as coreMigrateToVitest } from "@ts-dev-tools/core/dist/install/migrations/20260604100000-migrate-to-vitest";
...
import { up as init } from "./20201111162698-init";
import { up } from "./20260604100001-migrate-to-vitest";

const useCache = true;
const shouldCleanupAfterTest = true;

describe("Migration 20260604100001-migrate-to-vitest", () => {
  ...
  it("should migrate generated React Jest config and scripts to Vitest", async () => {
    await coreInit(testProjectDir);
    init(testProjectDir);
    await coreMigrateToBiome(testProjectDir);
    await coreMigrateToVitest(testProjectDir);
    ...
    expect(
      FileService.getFileContent(join(testProjectDir, "vitest.setup.ts")),
    ).toMatchSnapshot();
  });
